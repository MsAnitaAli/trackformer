...
# ==============================================================================
MODEL_BASE_NAME = "TrackEgoFormerAppearance"  
OUT_DIR = "egohumans_appearance2"
...
TRACKER_SUBPATH = "trackers/mot_challenge/Egohumans_full_MOT-TRAIN"
...
TARGET_SETUPS = {
    ...
    "Best MOTA (With ReID)":   {"folder": f"{MODEL_BASE_NAME}_ReID_best_MOTA_TRAIN", "color": COLOR_MOTA,  "row": 0},
    "Best IDF1 (With ReID)":   {"folder": f"{MODEL_BASE_NAME}_ReID_best_IDF1_TRAIN", "color": COLOR_IDF1,  "row": 0},
    "Final Epoch (With ReID)": {"folder": f"{MODEL_BASE_NAME}_ReID_TRAIN",           "color": COLOR_FINAL, "row": 0},
    
    # --- ROW 1: NO REID MODELS ---
    "Best MOTA (No ReID)":     {"folder": f"{MODEL_BASE_NAME}_NoReID_best_MOTA_TRAIN", "color": COLOR_MOTA,  "row": 1},
    "Best IDF1 (No ReID)":     {"folder": f"{MODEL_BASE_NAME}_NoReID_best_IDF1_TRAIN", "color": COLOR_IDF1,  "row": 1},
    "Final Epoch (No ReID)":   {"folder": f"{MODEL_BASE_NAME}_NoReID_TRAIN",           "color": COLOR_FINAL, "row": 1},
}
...
pdf_out = os.path.join(OUTPUT_GRAPH_DIR, "hota_curves_2x2_grid_train.pdf")
eps_out = os.path.join(OUTPUT_GRAPH_DIR, "hota_curves_2x2_grid_train.eps")
png_out = os.path.join(OUTPUT_GRAPH_DIR, "hota_curves_2x2_grid_train.png")
