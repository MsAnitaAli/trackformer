...
def main(seed, dataset_name, obj_detect_checkpoint_file, tracker_cfg,
    ...
    if obj_detector_model is None:
        # 1. CLEAR MOT Metrics
        _log.info(f"EVAL (CLEAR/MOT Metrics):")
        summary, str_summary = evaluate_mot_accums(
            mot_accums,
            [str(s) for s in dataset if not s.no_gt])
        _log.info(f'\n{str_summary}')

        # 2. HOTA Metrics via TrackEval
        _log.info(f"EVAL (HOTA Metrics):")
        try:
            eval_config = trackeval.Evaluator.get_default_eval_config()
            eval_config['PRINT_RESULTS'] = True
            eval_config['DISPLAY_LESS_PROGRESS'] = True
            
            # Extract split (TEST or VAL) from Egohumans_full-TEST
            split_name = dataset_name.split('-')[-1]
            benchmark = 'Egohumans'

            dataset_config = trackeval.datasets.MotChallenge2DBox.get_default_dataset_config()
            dataset_config.update({
                'GT_FOLDER': data_root_dir, 
                'TRACKERS_FOLDER': osp.join(output_dir, '..'), 
                'BENCHMARK': benchmark, 
            ...
            _log.info("HOTA Evaluation Complete.")
        except Exception as e:
            _log.info(f"HOTA Eval skipped or failed: {e}")

        # 3. Save Summary to File
        if output_dir is not None:
            timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M")
            summary_path = osp.join(
                output_dir, f"mot_results_{dataset_name}_{timestamp}.txt"
            )
            ...
            with open(summary_path, "w") as f:
                ...
                f.write(f"Checkpoint: {obj_detect_checkpoint_file}\n")
                f.write("\n")
                f.write(str_summary)
                f.write("\n")
            _log.info(f"MOT results saved to: {summary_path}")
